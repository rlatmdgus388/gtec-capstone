      ...
      <div className="p-4 space-y-4">
        {isLoading ? (
          ...
          <Card className="border border-border rounded-xl">
            <CardContent className="p-6 text-center text-muted-foreground">학습 기록이 없습니다.</CardContent>
          </Card>
        ) : (
          statsPeriods.map((stats, index) => {
            ...
            return (
              <Card
                key={index}
                className="bg-card shadow-md rounded-2xl cursor-pointer hover:bg-muted transition-colors"
                onClick={() =>
                  stats.totalWords > 0 &&
                  setDetailPageData({
              ...
              >
                <CardContent className="p-5">
                  <h3 className="font-semibold text-foreground text-lg mb-2">{stats.period} 학습한 단어</h3>
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="text-4xl font-bold text-foreground mb-3">{stats.totalWords}</p>
                      <div className="flex items-center gap-2">
                        <Badge className="bg-green-100 text-green-800 hover:bg-green-100">
                          정답 {stats.correctCount}
                        </Badge>
                        <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100">
                          오답 {stats.incorrectCount}
                        </Badge>
                      </div>
                    </div>
