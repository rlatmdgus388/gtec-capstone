...
export function SharedWordbooksScreen({ onBack, onSelectWordbook }: { onBack: () => void; onSelectWordbook: (wordbookId: string) => void; }) {
                ...
                <div className="bg-white shadow-sm border-b sticky top-0 z-10 p-4">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center">
                            ...
                            <h1 className="font-semibold text-gray-900 text-lg">공유 단어장</h1>
                        </div>
                        <Button size="sm" onClick={() => setIsShareDialogOpen(true)}><Share2 className="h-4 w-4 mr-2" /> 공유하기</Button>
                    </div>
                    <div className="relative mb-3">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                        <Input placeholder="단어장 이름 또는 작성자로 검색" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-10 bg-gray-50 border-gray-200 rounded-md" />
                    </div>
                    <ScrollArea className="w-full whitespace-nowrap"><div className="flex w-max space-x-2">
                        {FILTER_CATEGORIES.map((cat) => (<Badge key={cat.value} variant={selectedCategory === cat.value ? "default" : "secondary"} onClick={() => setSelectedCategory(cat.value)} className="cursor-pointer">{cat.label}</Badge>))}
                    </div><ScrollBar orientation="horizontal" /></ScrollArea>
                </div>
                <div className="p-4 space-y-4">
                    {isLoading ? <div className="space-y-3"><Skeleton className="h-24 w-full" /><Skeleton className="h-24 w-full" /><Skeleton className="h-24 w-full" /></div> : filteredWordbooks.length === 0 ? <Card className="text-center py-16 border-dashed"><CardContent><BookOpen size={48} className="mx-auto text-gray-300 mb-4" /><h3 className="text-lg font-semibold text-gray-900 mb-2">결과 없음</h3><p className="text-sm text-gray-600">검색 조건을 변경해보세요.</p></CardContent></Card> : (
                        filteredWordbooks.map(wordbook => (
                            // ✨ 아래 Card의 onClick 이벤트가 수정되었습니다.
                            <Card key={wordbook.id} onClick={() => onSelectWordbook(wordbook.id)} className="cursor-pointer transition-all hover:shadow-md">
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1">
                                            ...
                                            <p className="text-sm text-gray-500">{wordbook.wordCount} words</p>
                                            <Badge variant="secondary" className="mt-2">{wordbook.category}</Badge>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {currentUserId === wordbook.author.uid && (
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader><AlertDialogTitle>정말 삭제하시겠습니까?</AlertDialogTitle><AlertDialogDescription>이 작업은 되돌릴 수 없습니다. 단어장이 커뮤니티에서 영구적으로 삭제됩니다.</AlertDialogDescription></AlertDialogHeader>
                                                        <AlertDialogFooter>
                                                            <AlertDialogCancel onClick={e => e.stopPropagation()}>취소</AlertDialogCancel>
                                                            <AlertDialogAction onClick={(e) => { e.stopPropagation(); handleDelete(wordbook.id); }}>삭제</AlertDialogAction>
                                                        </AlertDialogFooter>
                                                    </AlertDialogContent>
                                                </AlertDialog>
                                            )}
                                            <Button size="sm" onClick={(e) => handleDownload(e, wordbook)}><Download className="h-4 w-4 mr-2" />다운로드</Button>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4 text-sm text-gray-500">
                                        <span className="flex items-center gap-1"><Heart size={14} />{wordbook.likes}</span>
                                        <span className="flex items-center gap-1"><Download size={14} />{wordbook.downloads}</span>
                                        <span className="flex items-center gap-1"><Eye size={14} />{wordbook.views}</span>
                                    </div>
                                </CardContent>
                                {/* ✨ 아래 펼치기 상세 정보 부분이 삭제되었습니다. */}
                            </Card>
                        ))
                    )}
                </div>
